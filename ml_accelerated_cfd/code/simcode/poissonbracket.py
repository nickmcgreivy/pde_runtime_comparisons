...
def interpolate_learned(zeta, model, params):
    s_R, s_T = model.apply(
        params, zeta
    ...
    zeta_pad_R = np.pad(
        zeta,
        (
            ((model.width_even - 1) // 2, (model.width_even - 1) // 2 + 1),
            ((model.width_odd - 1) // 2, (model.width_odd - 1) // 2),
            (0, 0),
        ),
        "wrap",
    )
    ...
    P_R = jax.lax.conv_general_dilated_patches(
        zeta_pad_R[None, ...],
        iter([model.width_even, model.width_odd]),
        iter([1, 1]),
        "VALID",
        dimension_numbers=("CHWN", "OIHW", "HWCN"),
    ).reshape(
        zeta.shape[0],
        zeta.shape[1],
        model.width_even,
    ...
    P_T = jax.lax.conv_general_dilated_patches(
        zeta_pad_T[None, ...],
        iter([model.width_odd, model.width_even]),
        iter([1, 1]),
        "VALID",
        dimension_numbers=("CHWN", "OIHW", "HWCN"),
    ).reshape(
    ...
    zeta_R = np.einsum("ijklba,ijklb->ija", s_R, P_R)
    zeta_T = np.einsum("ijklba,ijklb->ija", s_T, P_T)
    return zeta_R, zeta_T
...
def get_poisson_bracket(basedir, order, flux):
    ...
    def centered(zeta, H, model=None, params=None):
    ...
    def upwind(zeta, H, model=None, params=None):
        alpha_R_points, alpha_T_points = H @ R.T, H @ T.T  # right, top
    ...
    def vanleer(zeta, H, model=None, params=None):
    ...
    def conservation(zeta, H, model=None, params=None):
        assert zeta.shape[-1] == 1
        ...
        #B_xij = np.sum((zeta - np.roll(zeta, 1, axis=0)) * F_L - zeta**2 / 2 * (alpha_R_points - np.roll(alpha_R_points, 1, axis=0)),axis=-1)
        #B_yij = np.sum((zeta - np.roll(zeta, 1, axis=1)) * F_B - zeta**2 / 2 * (alpha_T_points - np.roll(alpha_T_points, 1, axis=1)),axis=-1)
    ...
    def learned(zeta, H, model=None, params=None):
        alpha_R_points, alpha_T_points = H @ R.T, H @ T.T
        zeta_R_leg, zeta_T_leg = interpolate_learned(zeta, model, params)
        zeta_R_points, zeta_T_points = zeta_R_leg @ LP, zeta_T_leg @ LP
        interp_R_leg = (alpha_R_points * zeta_R_points) @ P_inv.T
        interp_T_leg = (alpha_T_points * zeta_T_points) @ P_inv.T
        return interp_R_leg, interp_T_leg
    ...
    def poisson_bracket(zeta, H, model=None, params=None):
        interp_R, interp_T = interp_func(zeta, H, model=model, params=params)
