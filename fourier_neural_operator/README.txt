...
This appears to be inaccurate. The DG code (on laptop CPU) is ~16x slower than the 5ms ML code, not 3 orders of magnitude slower.

I'm not sure how much faster the DG code would be on a GPU. The runtime on GPU probably dominated by the sparse linear solve, which as far as I know is probably a few times faster than the CPU sparse solve but not dramatically faster. So even on GPU, the ML code is probably somewhere in the range of 2-16x faster. Maybe Ammar can comment on this.
...
FNO uses the 2D incompressible Navier-Stokes equation (i.e., incompressible euer equation) with nu = 1e-3, 1e-4, 1e-5, x,y = [0,1], t = [0, T], and forcing function f(x,y) = 0.1 * (sin( 2 pi (x + y)  ) + cos( 2 pi (x + y) )
